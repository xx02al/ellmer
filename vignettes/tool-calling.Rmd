...
A tool sometimes needs to know a little about the call it's part of — the tool
request that triggered it, or the conversation so far — without that information
being part of its arguments. Inside a tool body, `tool_context()` returns a
...
see — a database connection, an authenticated user, or a running tally. A clean
way to organise this is to bundle the tools together as methods of an R6 class,
with the shared state living in the object's fields. Each tool reads and writes

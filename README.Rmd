...
ellmer supports a wide variety of model providers. Official providers are
actively maintained, with priority support for bug fixes and new features.
Community providers are contributed and maintained by the community;
contributions to improve them are especially welcome.
...
- `chat_openai()` or `chat_anthropic()` are good places to start. `chat_openai()` defaults to **GPT-5.6 Terra**, but you can use `model = "gpt-5.6-luna"` for a cheaper, faster model. `chat_anthropic()` defaults to **Claude Sonnet 5**, which we have found to be particularly good at writing R code.

- `chat_google_gemini()` is a strong model with a free tier (with the downside that [your data is used](https://ai.google.dev/gemini-api/terms#unpaid-services) to improve the model), making it a great place to start if you don't want to spend any money.
...
* Learn tips for programming with ellmer, including inside your own package, in `vignette("programming")`.

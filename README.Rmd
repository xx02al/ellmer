...
- `chat_openai()` or `chat_anthropic()` are good places to start. `chat_openai()` defaults to **GPT-5.4**, but you can use `model = "gpt-5.4-nano"` for a cheaper, faster model. `chat_anthropic()` defaults to **Claude Sonnet 4.6**, which we have found to be particularly good at writing R code.
